...
class GestorRecetas: #  define los objetos para usarlos mas adelante  
    def __init__(self):  # inicia los atributos de un objeto 
        self.recetas = []
...
def agregar_receta():
    ...
    ingredientes = entry_ingredientes.get().split(",") #divide la cadena en subcadenas y devuelve una lista de subcadenas
...
# Menú
menubar = Menu(ventana) # crea la ventana principal
...
menu_archivo.add_command(label="Salir", command=salir)# muestra un menu en la parte superior 
menubar.add_cascade(label="Inicio", menu=menu_archivo)

operaciones_menu = tk.Menu(menubar, tearoff=0)
operaciones_menu.add_command(label="agregar receta", command=agregar_receta)
operaciones_menu.add_command(label="Ver ingredientes", command=ver_ingredientes)
operaciones_menu.add_command(label="Eliminar receta", command=eliminar_receta)
menubar.add_cascade(label="Menu", menu=operaciones_menu)
...
btn_eliminar = tk.Button(ventana, text=" salir", command=salir)
btn_eliminar.pack()
